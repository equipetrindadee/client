...
import '../buttonFiltrarListar/filterButtonListarMaster.css';

function FilterButtonListarMaster({ onFilterChange, colunas }) {
  ...
  const handleItemClick = (item) => {
    ...
    if (item === 'Mostrar todas as colunas') {
      ...
      onFilterChange('asc'); // Ordena todos em ordem crescente
    } else if (item === 'Z-A') {
      onFilterChange('desc'); // Ordena todos em ordem decrescente
    } else {
      onFilterChange(item);  // Passa a categoria selecionada (Aluno ou Professor)
    }
  };
...
export default FilterButtonListarMaster;

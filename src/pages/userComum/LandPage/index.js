...
export const LandPage = () => {
  ...
  const handleClick = (colunaName) => {
    // Verifica se as colunas já foram carregadas
    if (!colunas || colunas.length === 0) {
      console.log("Colunas ainda não carregadas.");
      return;
    }
    ...
    if (colunaSelecionada) {
      // Salva as informações no localStorage para uso posterior
      ...
      console.log(`Coluna não encontrada: ${colunaName}`);
